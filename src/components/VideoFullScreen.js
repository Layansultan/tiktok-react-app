...
export default function VideoFullScreen({ video, onBack, onOptions }) {
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 40, background: "#000" }}>
      {/* gradient background */}
      <div style={{ position: "absolute", inset: 0, background: video.thumb }} />
      ...
      <div style={{ position: "absolute", right: 10, bottom: 120, display: "flex", flexDirection: "column", alignItems: "center", gap: 18, zIndex: 50 }}>
        {/* avatar */}
        <div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#fe2c55,#ff9500)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "white" }}>L</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
          ...
          <span style={{ fontSize: 9, color: "white", fontWeight: 600 }}>{video.views}</span>
        </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "12px 14px 14px", background: "linear-gradient(transparent, rgba(0,0,0,0.7))", zIndex: 50 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "white", marginBottom: 3 }}>@layan</div>
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.8)", marginBottom: 8 }}>{video.title}</div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.1)", borderRadius: 20, padding: "6px 14px" }}>
          <span style={{ fontSize: 11, color: "white" }}>▶ {video.views} views</span>
