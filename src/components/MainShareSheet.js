    ...
    <Sheet height={460}>
      ...
      <div style={{ display: "flex", overflowX: "auto", padding: "10px 10px", gap: 4, scrollbarWidth: "none" }}>
        <div onClick={onRepost} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, minWidth: 52, position: "relative", cursor: "pointer" }}>
          <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#333", border: "1.5px solid white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17 }}>🔁</div>
          <span style={{ fontSize: 8, color: "white", fontWeight: 700 }}>{alreadyReposted ? "✓ Reposted" : "Repost"}</span>
        </div>
      ...
      <div style={{ display: "flex", overflowX: "auto", padding: "10px 10px 6px", gap: 4, scrollbarWidth: "none" }}>
        <div onClick={onDuet} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, minWidth: 52, cursor: "pointer" }}>
          <div style={{ width: 42, height: 42, borderRadius: "50%", background: C.surf2, border: `1.5px solid #7ba7f7`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, boxShadow: "0 0 8px rgba(123,167,247,0.45)" }}>👥</div>
          <span style={{ fontSize: 8, color: "#7ba7f7", fontWeight: 700 }}>Duet</span>
        </div>
        <div onClick={onStitch} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, minWidth: 52, cursor: "pointer" }}>
          <div style={{ width: 42, height: 42, borderRadius: "50%", background: C.surf2, border: `1.5px solid ${C.amber}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, boxShadow: `0 0 8px rgba(245,158,11,0.35)` }}>✂</div>
          <span style={{ fontSize: 8, color: C.amber, fontWeight: 700 }}>Stitch</span>
        </div>
      ...
      <div style={{ margin: "8px 12px 10px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ flex: 1, fontSize: 10, color: "rgba(255,255,255,0.8)", lineHeight: 1.5 }}>
          <span style={{ color: C.grey, fontWeight: 700 }}>Repost</span> may reach audiences beyond the original context ·{" "}
          <span style={{ color: "#7ba7f7", fontWeight: 700 }}>Duet</span> &amp; <span style={{ color: C.amber, fontWeight: 700 }}>Stitch</span> use the creator's content
        </div>
